...
CryptoPipe is a production-style ELT pipeline for automated crypto market monitoring. It pulls live market data from CoinGecko, lands the raw response in Google Cloud Storage and BigQuery, and uses dbt to build an analysis-ready table for reporting in Looker Studio.

## What It Does

Crypto market data changes quickly, is highly nested, and is not immediately useful for reporting. Querying an API on demand is inefficient, and analyzing the raw response directly makes it harder to track trends, compare coins, or build dashboards.

CryptoPipe automates that path end to end. It centralizes ingestion, preserves the raw data in cloud storage, stages the same data in BigQuery, and applies dbt business logic to calculate metrics such as volume efficiency and market-cap tiers. The result is a repeatable pipeline that is easier to audit, scale, and present.

This project is intentionally ELT, not ETL. The raw data is loaded first, and the transformation happens later inside the warehouse with dbt.

## Architecture

```mermaid
flowchart TB
    api[CoinGecko API\nTop 10 market-cap coins] --> mage[Mage.ai in Docker\nPython loader block]
    mage --> gcs[GCS data lake\nParquet export]
    mage --> bq[BigQuery raw staging\ntop_crypto_prices]
    gcs --> dbt[dbt model\nstg_crypto_metrics\nDaily partitioning]
    bq --> dbt
    dbt --> looker[Looker Studio dashboard]
```
...
| Google Cloud Storage | Stores the exported Parquet file as a data lake layer. |
| Google BigQuery | Holds both the raw staging table and the dbt model output. |
| CoinGecko API | Supplies live market data for the top cryptocurrencies. |
| dbt | Handles SQL transformation, type casting, metric calculation, and partitioned table creation. |
| Looker Studio | Provides the reporting layer for dashboards and analysis. |
...
- A GCP service account JSON key with permissions for Storage and BigQuery
- The service account file saved at the repository root as `creds.json`

### Configure Environment

If you are setting this up for your own GCP account, these files matter most:

- `.env.example`: copy to `.env` in the repository root and fill in your values.
- `.env`: keep active environment variables here in the repository root.
...
- `default_repo/data_exporters/export_to_gcs.py`: reads `GCS_BUCKET_NAME` so the block code can stay unchanged.
- `default_repo/data_exporters/export_to_bigquery.py`: reads `GOOGLE_CLOUD_PROJECT_ID`, `BIGQUERY_DATASET`, and `BIGQUERY_TABLE_NAME`.

Recommended approach: keep shared variables in `.env`, then leave the code unchanged.
...
1. Open a second shell in the container.
...
Then enter the pipeline named `cryptopipe-pipeline`.
